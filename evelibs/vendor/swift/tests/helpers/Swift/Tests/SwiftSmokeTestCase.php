...
class Swift_Tests_SwiftSmokeTestCase extends Swift_Tests_SwiftUnitTestCase
{
    ...
    protected function _getMailer()
    {
        switch (SWIFT_SMOKE_TRANSPORT_TYPE) {
            case 'smtp':
                $transport = Swift_DependencyContainer::lookup('transport.smtp')
                ...
                break;
            case 'sendmail':
                $transport = Swift_DependencyContainer::lookup('transport.sendmail')
                    ->setCommand(SWIFT_SMOKE_SENDMAIL_COMMAND)
                    ;
                break;
            case 'mail':
            case 'nativemail':
                $transport = Swift_DependencyContainer::lookup('transport.mail');
                break;
            default:
