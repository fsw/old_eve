...
class field_File extends Field
{
	...
	public static function fromFile($path, $name = null)
	{
		...
		Fs::copyr($path, Site::getDataDir() . $video['name'] . '.' . $video['type']);
		...
		switch ($video['type'])
		{
			case 'mp4' :
				foreach (array('480x270' => '_t.jpg', '170x96' => '_ts.jpg') as $tsize => $tsufix)
				{
					...
					{
						$cmd = 'ffmpeg -y -ss ' . $sec .' -i ' . Site::getDataDir() . $video['name'] . '.mp4 -vframes 1 -s ' . $tsize . ' ' . Site::getDataDir() . $video['name'] . $tsufix;
						exec($cmd);
						if (Fs::exists(Site::getDataDir() . $video['name'] . $tsufix))
						{
							break;
						}
					}
				...
				$cmd = 'ffmpeg -y -i ' . Site::getDataDir() . $video['name'] . '.mp4 ' . Site::getDataDir() . $video['name'] . '.webm';
				exec($cmd);
			...
			case 'jpg' :
				...
				static::saveGdThumb($source_image, Site::getDataDir() . $video['name'] . '_t.jpg');
				break;
			case 'png' :
